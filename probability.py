...
def equation_second_degre():
    ...
    if user_delta is None or user_delta != delta:
        ...
        if delta < 0:
            ...
            solution = -b / (2 * a)
            print_red(f"L'équation admet une unique solution : x = {convert_to_fraction(solution)}")
        else:
    ...
    if delta < 0:
        ...
        if num_solutions == expected_num_solutions:
            ...
            if user_solution == convert_to_fraction(solution):
                print_green(f"Bonne réponse ! L'équation admet une unique solution : x = {convert_to_fraction(solution)}\n")
            else:
                print_red(f"Mauvaise réponse. La solution est : x = {convert_to_fraction(solution)}")
                points = 0
        else:
            points = 0
            print_red(f"Mauvaise réponse. L'équation admet une unique solution : x = {convert_to_fraction(-b / (2 * a))}\n")
    else:
        ...
        if num_solutions == expected_num_solutions:
            ...
            points = 0
            print_red(f"Mauvaise réponse. L'équation admet deux solutions : x1 et x2.\n")
...
def integration():
    ...
    if choix == "Fonctions puissance":
        ...
        if sous_choix == "f(x) = (cx - d)^α":
            ...
            user_answer = Fraction(input("Calculez l'intégrale de la fonction donnée : I = ∫(cx - d)^α dx, avec x de a={} à b={}, c={}, d={} et alpha={}: ".format(a, b, c, d, alpha)))
            if math.isclose(user_answer.real, result.real, rel_tol=1e-6):
                print_green(f"Bravo, votre réponse est correcte ! Le résultat de l'intégrale est :{result}")
                points = 1
            else:
                print_red(f"Désolé, votre réponse est incorrecte. Le résultat de l'intégrale est :{result}")
            ...
            user_answer = Fraction(input("Calculez l'intégrale de la fonction donnée : I = ∫1 / (x - c) dx, avec x de a={} à b={} et c={}: ".format(a, b, c)))
        ...
        if sous_choix == "f(x) = cos(cx)":
            ...
            user_answer = float(input("Calculez l'intégrale de la fonction donnée : I = ∫cos(cx) dx, avec x de a={} à b={} et c={}: ".format(a, b, c)))
            if math.isclose(user_answer, result, rel_tol=1e-6):
                print_green(f"Bravo, votre réponse est correcte ! Le résultat de l'intégrale est :{result}")
                points = 1
            else:
            ...
            user_answer = Fraction(input("Calculez l'intégrale de la fonction donnée : I = ∫sin(cx) dx, avec x de a={} à b={} et c={}: ".format(a, b, c)))
            if math.isclose(user_answer, result, rel_tol=1e-6):
                print_green(f"Bravo, votre réponse est correcte ! Le résultat de l'intégrale est :{result}")
                points = 1
            else:
            ...
            user_answer = Fraction(input("Calculez l'intégrale de la fonction donnée : I = ∫tan(cx) dx, avec x de a={} à b={} et c={}: ".format(a, b, c)))
            if math.isclose(user_answer, result, rel_tol=1e-6):
                print_green(f"Bravo, votre réponse est correcte ! Le résultat de l'intégrale est :{result}")
                points = 1
            else:
        ...
        user_answer = Fraction(input("Calculez l'intégrale de la fonction donnée : I = ∫ln(cx) dx, avec x de a={} à b={} et c={}: ".format(a, b, c)))
        if math.isclose(user_answer, result, rel_tol=1e-6):
            print_green(f"Bravo, votre réponse est correcte ! Le résultat de l'intégrale est :{result}")
            points = 1
        else:
...
def main():
    ...
    print_yellow(f"Vous avez choisi {nombre_questions} question{'s' if nombre_questions > 1 else ''}, c'est parti !")
